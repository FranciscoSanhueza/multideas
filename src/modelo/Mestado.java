...
public class Mestado extends conexionBD {
    ...
    public String SelectEstado(ArrayList<estado> ARLest) {
        if (super.obtenerConexion() != null) {
            try {
                ps = super.obtenerConexion().prepareStatement("select * from estado");
                rs = ps.executeQuery();
                rs.last();
                if (rs.getRow() > 0) {
                    rs.beforeFirst();
                    while (rs.next()) {
                        ARLest.add(new estado(rs.getByte(1), rs.getString(2)));
                    }
                    return "cargado";
